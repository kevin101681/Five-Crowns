...
import {
  ...
  Edit2, Trash2, Info, Trophy, Target, TrendingUp,
} from "lucide-react";
...
export default function SetupScreen({ onStart }: SetupScreenProps) {
  ...
  const winRate = (stats: PlayerStats) =>
    stats.gamesPlayed > 0
      ? Math.round((stats.gamesWon / stats.gamesPlayed) * 100)
      : null;

  const avgScore = (stats: PlayerStats) =>
    stats.gamesPlayed > 0
      ...
      {!dbConfigured && (
        <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-2xl text-amber-500 text-xs mb-3">
          ...
          <p className="opacity-80">
            Players will not be saved between sessions. Set{" "}
            <code className="bg-amber-500/20 px-1 rounded">DATABASE_URL</code> in Settings.
          </p>
        </div>
      )}
        ...
        <AnimatePresence mode="popLayout">
          {players.length === 0 ? (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="w-full text-center py-16 border-2 border-dashed border-outline/10 rounded-3xl"
            >
              ...
              {players.map((player) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`relative flex flex-col items-center gap-2 p-4 rounded-2xl border cursor-pointer transition-colors select-none ${
                      isDealer
                        ...
                        : isSelected
                        ? "border-primary/30 bg-surface-variant"
                  ...
                  >
                    ...
                    {/* Avatar */}
                    <div
                      className={`w-16 h-16 rounded-full flex items-center justify-center overflow-hidden border-2 mt-2 ${
                        isDealer
                          ? "border-primary"
                          : isSelected
                          ? "border-primary/40"
                    ...
                    {/* Name */}
                    <span
                      className={`font-bold text-sm text-center truncate max-w-full px-1 ${
                        isDealer ? "text-primary" : "text-on-surface"
                      }`}
                    >
                      {player.name}
                    </span>
                    ...
                    {/* Dealer badge */}
                    {isDealer && (
                      <span className="text-[9px] font-black uppercase text-primary tracking-widest -mt-1">
                        Dealer
                      </span>
                    )}
            ...
            >
              ...
              <div className="px-5 pb-5 space-y-5">
                ...
                <div className="flex flex-col items-center gap-3">
                  <div className="relative group">
                    <div
                      onClick={() => isEditingProfile && editFileInputRef.current?.click()}
                      className={`w-20 h-20 rounded-full bg-primary/10 border-2 border-primary/30 flex items-center justify-center overflow-hidden ${isEditingProfile ? "cursor-pointer" : ""}`}
                    >
                      {(isEditingProfile ? editAvatar : profilePlayer.avatarUrl) ? (
                        <img
                          src={isEditingProfile ? editAvatar : profilePlayer.avatarUrl}
                  ...
                  {isEditingProfile ? (
                    <input
                      ...
                      className="text-center font-black text-xl w-full px-3 py-2 rounded-xl border border-outline bg-surface-variant text-on-surface focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all"
                    />
                  ) : (
                  ...
                  return (
                    <div className="grid grid-cols-2 gap-2">
                      <div className="bg-surface-variant rounded-2xl p-3 border border-outline/10">
                        <div className="flex items-center gap-1.5 text-on-surface-variant/60 text-[10px] font-bold uppercase tracking-wider mb-1">
                          <Target size={10} /> Games
                        </div>
                      ...
                      {stats.bestScore !== null && (
                        <div className="col-span-2 bg-primary/10 rounded-2xl p-3 border border-primary/20">
                          <div className="flex items-center gap-1.5 text-primary/70 text-[10px] font-bold uppercase tracking-wider mb-1">
                            <Trophy size={10} /> Best Score (Lowest)
                          </div>
                ...
                {isEditingProfile ? (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-3 rounded-2xl border border-outline/20 font-bold text-on-surface-variant hover:bg-surface-variant transition-all active:scale-95"
                    >
                      Cancel
                    </button>
                    <button
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-3 rounded-2xl border border-outline/20 font-bold text-on-surface flex items-center justify-center gap-2 hover:bg-surface-variant transition-all active:scale-95"
                    >
                      <Edit2 size={15} /> Edit
                    </button>
                    <button
                      onClick={() => {
                        if (confirm(`Remove ${profilePlayer.name}? Their stats will also be deleted.`)) {
                          removePlayer(profilePlayer.id);
                        }
                      }}
